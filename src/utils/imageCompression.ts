 ...
 * @param options - Compression options
 * @param onProgress - Progress callback (current, total, currentFile)
 * @returns Array of compressed results
 */
...
/** File with optional size properties */
interface SizedFile {
  size?: number;
  compressedSize?: number;
}
 ...
 */
export const getTotalSize = (files: FileList | File[] | SizedFile[]): number => {
  return Array.from(files as SizedFile[]).reduce(
    (total, file) => total + (file.size || (file as CompressionResult['info']).compressedSize || 0),
    0
  );
};
