...
export const frameSizes: Record<
  string,
  { name: string; ratio: string; width: number; height: number; spec: string; platforms: string; category: string }
> = {
...
export function getFrameSizesByCategory(category: string): Record<string, (typeof frameSizes)[string]> {
  return Object.fromEntries(Object.entries(frameSizes).filter(([_, size]) => size.category === category));
}
