...
export default function useTabs(initialTabs = [], user = null) {
  ...
  const handleUpdateProjectName = (newName) => {
    ...
    const nameExists = projects.some((p) => p.id !== activeTabId && p.name.toLowerCase() === trimmedName.toLowerCase());
    ...
    setProjects((prev) =>
      prev.map((p) =>
        p.id === activeTabId
          ? { ...p, name: trimmedName, updatedAt: new Date().toISOString().split('T')[0], lastEditedBy: getUserEmail() }
          : p
      )
    );
  ...
  const handleRenameProject = (projectId, newName) => {
    ...
    // Check if name already exists (excluding current project)
    const nameExists = projects.some((p) => p.id !== projectId && p.name.toLowerCase() === trimmedName.toLowerCase());
