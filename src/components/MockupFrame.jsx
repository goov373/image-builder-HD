      ...
      <div className="w-full h-full bg-[#0f172a] p-4 flex">
        ...
        <div className="w-48 bg-gray-800/50 rounded-lg p-3 mr-4">
          {['General', 'Security', 'Billing', 'Team', 'API'].map((item, i) => (
            <div 
              key={item} 
              className={`px-3 py-2 rounded-lg mb-1 ${i === 0 ? 'bg-gray-700' : ''}`}
            >
        ...
        <div className="bg-gray-800/30 rounded-lg overflow-hidden">
          ...
          {[1,2,3,4,5].map(i => (
            <div key={i} className="flex items-center gap-4 px-4 py-3 border-t border-gray-800">
              ...
              <div className="h-5 w-12 rounded" style={{ backgroundColor: i === 2 ? '#10b981' : '#3b82f6', width: '15%' }} />
...
const MockupFrame = ({
  ...
  const buildShadow = () => {
    ...
    if (style.shadowEnabled) {
      shadows.push(`${style.shadowX || 0}px ${style.shadowY || 24}px ${style.shadowBlur || 48}px ${style.shadowSpread || -12}px ${style.shadowColor || 'rgba(0,0,0,0.25)'}`);
    }
