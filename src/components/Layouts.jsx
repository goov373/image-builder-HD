...
// Common text container styles for overflow control
const textContainerBase = 'overflow-hidden';
...
export const LayoutBottomStack = ({ headline, body, text, accent, isLandscape, headingFont, bodyFont, variant = 0, isFrameSelected, onUpdateText, activeField, onActivateField, formatting = {}, fontSizes = {} }) => {
  ...
  if (isLandscape) {
    return (
      <div className={`absolute inset-0 flex ${getLandscapeAlignment()} px-3 py-3 ${getLandscapeGradient()} overflow-hidden`}>
        <div className={`flex items-center justify-between w-full gap-4 ${textContainerBase}`}>
  ...
  return (
    <div className={`absolute inset-0 flex flex-col ${getAlignment()} p-4 ${getGradient()} overflow-hidden`}>
      <div className={`relative mb-1.5 w-full ${textContainerBase}`}>
      ...
      <div className={`relative w-full ${textContainerBase}`}>
...
export const LayoutCenterDrama = ({ headline, body, text, accent, isLandscape, headingFont, bodyFont, variant = 0, isFrameSelected, onUpdateText, activeField, onActivateField, formatting = {}, fontSizes = {} }) => {
  ...
  if (isLandscape) {
    return (
      <div className={`absolute inset-0 flex ${getLandscapeAlignment()} justify-center px-3 py-3 ${getLandscapeGradient()} overflow-hidden`}>
        <div className={`flex items-center gap-4 max-w-[95%] ${textContainerBase}`}>
          <EditableTextField {...commonProps} field="headline" isActive={activeField === 'headline'} formatting={formatting.headline || {}} className="font-black leading-tight tracking-tight block flex-1 min-w-0" style={headlineStyle}>{headline}</EditableTextField>
          <div className="w-0.5 h-8 rounded-full flex-shrink-0" style={{ background: accent, opacity: 0.6 }} />
          <EditableTextField {...commonProps} field="body" isActive={activeField === 'body'} formatting={formatting.body || {}} className="text-gray-300 leading-snug font-medium block max-w-[35%] min-w-0" style={bodyStyle}>{body}</EditableTextField>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={`absolute inset-0 flex flex-col items-center ${getAlignment()} p-4 text-center bg-black/40 overflow-hidden`}>
      <div className={`max-w-[90%] w-full ${textContainerBase}`}>
        <div className={`relative mb-3 w-full ${textContainerBase}`}>
          <EditableTextField {...commonProps} field="headline" isActive={activeField === 'headline'} formatting={formatting.headline || {}} className="font-black leading-tight tracking-tight block w-full" style={headlineStyle}>{headline}</EditableTextField>
        </div>
        <div className="w-12 h-0.5 mx-auto mb-3 rounded-full" style={{ background: accent, opacity: 0.6 }} />
        <div className={`relative w-full ${textContainerBase}`}>
          <EditableTextField {...commonProps} field="body" isActive={activeField === 'body'} formatting={formatting.body || {}} className="text-gray-300 leading-relaxed font-medium block w-full" style={bodyStyle}>{body}</EditableTextField>
        </div>
      </div>
    </div>
  );
...
export const LayoutEditorialLeft = ({ headline, body, text, accent, isLandscape, headingFont, bodyFont, variant = 0, isFrameSelected, onUpdateText, activeField, onActivateField, formatting = {}, fontSizes = {} }) => {
  ...
  if (isLandscape) {
    return (
      <div className={`absolute inset-0 flex items-center ${getLandscapeContentAlign()} p-3 gap-3 overflow-hidden`}>
        ...
        <div className={`relative bg-black/40 backdrop-blur-sm rounded px-2 py-1 max-w-[35%] min-w-0 ${textContainerBase}`}>
  ...
  return (
    <div className={`absolute inset-0 flex flex-col ${getHeadlineAlign()} p-4 pt-8 overflow-hidden`}>
      <div className={`w-8 h-1 rounded-full mb-3 flex-shrink-0 ${getAccentAlign()}`} style={{ background: accent }} />
      <div className={`relative max-w-[85%] w-full mb-auto ${textContainerBase}`}>
        <EditableTextField {...commonProps} field="headline" isActive={activeField === 'headline'} formatting={formatting.headline || {}} className="font-bold leading-tight block w-full" style={headlineStyle}>{headline}</EditableTextField>
      </div>
      <div className={`relative ${getBodyAlign()} max-w-[75%] bg-black/30 backdrop-blur-sm rounded-lg p-2 ${textContainerBase}`}>
