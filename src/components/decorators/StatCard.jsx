...
const StatCard = ({
  ...
  const variants = {
    ...
    gradient: {
      bg: 'linear-gradient(135deg, #6466e9 0%, #818cf8 100%)',
      border: 'transparent',
      text: '#ffffff',
  ...
  return (
    ...
    >
      {/* Label */}
      <div className={`${s.label} font-medium uppercase tracking-wide mb-1`} style={{ color: v.label }}>
        {label}
      </div>

      {/* Value Row */}
      <div className="flex items-end justify-between">
        <span className={`${s.value} font-bold`} style={{ color: v.text }}>
          {value}
        </span>
