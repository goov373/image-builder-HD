...
const VideoCoverEditor = ({
  ...
  return (
    <div 
      ...
      onClick={(e) => { e.stopPropagation(); onSelect?.(videoCover.id); }}
    >
      {/* Header */}
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <button 
            type="button"
            onClick={(e) => { e.stopPropagation(); onSelect?.(isSelected ? null : videoCover.id); }} 
          ...
          <div>
            <div className="flex items-center gap-2">
              <h2 className={`text-lg font-bold transition-colors ${isSelected ? 'text-orange-400' : 'text-white'}`}>
                {videoCover.name}
              </h2>
              {isSelected && (
        ...
        <div className="mb-6 flex items-center gap-2 flex-wrap">
          ...
          {videoSizes.map(size => (
            <button
              key={size.key}
              type="button"
              onClick={(e) => { e.stopPropagation(); onChangeFrameSize?.(videoCover.id, size.key); }}
        ...
        <div className="mb-6 flex items-center gap-4">
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              ...
              onChange={(e) => { e.stopPropagation(); onTogglePlayButton?.(videoCover.id); }}
          ...
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              onChange={(e) => { e.stopPropagation(); onUpdateEpisodeNumber?.(videoCover.id, e.target.value); }}
              onClick={(e) => e.stopPropagation()}
              placeholder="Ep. 01"
              className="w-20 px-2 py-1 bg-gray-800 border border-gray-700 rounded text-xs text-white placeholder-gray-600 focus:outline-none focus:border-orange-500"
            />
          </div>
        </div>
      )}
